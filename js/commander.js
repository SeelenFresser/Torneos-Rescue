...
const AudioFX = (() => {
  ...
  return {
    ...
    roundStart() {
      [0, 0.12, 0.24].forEach((d, i) => beep([523, 659, 784][i], 0.18, 'triangle', 0.35, d));
    },
    roundEnd() {
      [0, 0.15, 0.3].forEach((d, i) => beep([784, 659, 523][i], 0.18, 'triangle', 0.3, d));
    },
    victory() {
      const notes = [523,659,784,1047,784,1047,1319];
      const times = [0,0.12,0.24,0.36,0.52,0.64,0.8];
      notes.forEach((n,i) => beep(n, 0.22, 'triangle', 0.4, times[i]));
    }
  };
})();
